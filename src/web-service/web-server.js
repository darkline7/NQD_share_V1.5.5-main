...
import { applyRuntimeControlsFromConfig, updateRuntimeControls } from "./runtime-controls.js";
import { getMediaQueueSettings, getMediaQueueSnapshot } from "../utils/media-upload-queue.js";
import { getCircuitBreakerSettings, getCircuitBreakerSnapshot } from "../utils/circuit-breaker.js";
import { getCloudflareAiConfig, getCloudflareAiStatus, updateCloudflareAiConfig } from "../utils/cloudflare-ai.js";
import { getAiMemorySnapshot } from "../utils/ai-memory.js";
...
export async function startWebServer(api) {
  ...
  applyRuntimeControlsFromConfig();

  app.use(express.static(path.join(__dirname, "../../public")));
  app.use(express.json());

  app.get("/api/runtime-controls", (req, res) => {
    res.json({
      mediaQueue: {
        settings: getMediaQueueSettings(),
        stats: getMediaQueueSnapshot(),
      },
      circuitBreaker: {
        settings: getCircuitBreakerSettings(),
        states: getCircuitBreakerSnapshot(),
      },
    });
  });

  app.post("/api/runtime-controls", (req, res) => {
    try {
      const updated = updateRuntimeControls(req.body || {});
      res.json({
        success: true,
        runtimeControls: updated,
      });
    } catch (error) {
      res.status(400).json({
        success: false,
  ...
  app.post("/api/ai-config", (req, res) => {
    try {
      const updated = updateCloudflareAiConfig(req.body || {});
      res.json({
        success: true,
        aiConfig: {
  ...
  app.get("/api/ai-status", (req, res) => {
    res.json({
      cloudflare: getCloudflareAiStatus(),
      memory: getAiMemorySnapshot(),
      queue: getMediaQueueSnapshot(),
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("getRuntimeControls", () => {
      socket.emit("runtimeControls", {
        mediaQueue: {
          settings: getMediaQueueSettings(),
          stats: getMediaQueueSnapshot(),
        },
    ...
    socket.on("updateRuntimeControls", (payload = {}) => {
      try {
        const updated = updateRuntimeControls(payload);
        socket.emit("runtimeControlsUpdated", {
          success: true,
          runtimeControls: updated,
        });
      } catch (error) {
    ...
    socket.on("updateAiConfig", (payload = {}) => {
      try {
        const updated = updateCloudflareAiConfig(payload);
        ...
        socket.emit("aiConfigUpdated", { success: false, message: error.message });
      }
    });
  });
