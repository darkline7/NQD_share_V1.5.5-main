...
class Zalo {
  ...
  async login() {
    ...
    if (!loginPayload || typeof loginPayload !== "object") {
      const loginError = loginData?.error || loginData?.message || "Dữ liệu đăng nhập rỗng";
      throw new Error(`Failed to login: ${loginError}. Vui lòng kiểm tra lại cookie/session Zalo.`);
    }

    const { zpw_enk, uid, zpw_service_map_v3, zpw_ws } = loginPayload;
    if (!zpw_enk || !uid || !zpw_service_map_v3 || !Array.isArray(zpw_ws) || !zpw_ws[0]) {
      throw new Error("Failed to login: thiếu trường bắt buộc trong phản hồi đăng nhập. Vui lòng kiểm tra lại cookie/session Zalo.");
    }

    appContext.secretKey = zpw_enk;
    appContext.uid = uid;
    setBotId(uid);
