      ...
      const commandCategories = {
        "Lệnh Quản trị": {
          "Quản lý Bot": commands.filter(
            (cmd) =>
              cmd.type === 3 &&
              ["prefix", "learn", "learnnow", "unlearn", "reply", "undo"].includes(cmd.name) &&
              cmd.active
          ),
          ...
          "Quản lý Thành viên": commands.filter(
            (cmd) => cmd.type === 3 && ["mute", "unmute", "listmute"].includes(cmd.name) && cmd.active
          ),
        ...
        "Lệnh Thành viên": {
          "Thông tin & Tiện ích": commands.filter(
            (cmd) =>
              cmd.type === 1 &&
              ["help", "ai", "info", "card", "group", "detail"].includes(cmd.name) &&
              cmd.active
          ),
          "Chat & Tương tác": commands.filter(
            (cmd) => cmd.type === 1 && ["voice", "sticker"].includes(cmd.name) && cmd.active
          ),
          "Âm nhạc": commands.filter(
            (cmd) => cmd.type === 1 && ["soundcloud"].includes(cmd.name) && cmd.active
          ),
        },
      };
